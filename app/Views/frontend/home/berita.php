  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="grid md:grid-cols-2 gap-12">
      ...
      <div>
        ...
        <h3 class="text-lg font-bold text-blue-600 mt-1 border-l-4 border-blue-600 pl-4">
          Agenda & Pengumuman
        </h3>
          ...
          if (! empty($pengumuman)) { ?>
            <?php foreach ($pengumuman as $row) { ?>
              <div class="bg-white rounded-lg overflow-hidden shadow-md p-6 flex flex-col md:flex-row">
                <div class="md:w-1/4 mb-4 md:mb-0 flex flex-col items-center justify-center bg-blue-100 rounded-lg p-4 md:mr-6">
                  <span class="text-3xl font-bold text-blue-600"><?= date('j', strtotime($row['created_at'])) ?></span>
                  <span class="text-blue-600 font-medium"><?= date('F', strtotime($row['created_at'])) ?></span>
                  <span class="text-blue-600"><?= date('Y', strtotime($row['created_at'])) ?></span>
                </div>
                <div class="md:w-3/4">
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    <a href="<?= base_url('post/' . $row['post_slug']) ?>" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Selengkapnya</a>
                  </div>
                </div>
              </div>
            <?php } ?>
            ...
            <?php foreach ($jurnal as $row) { ?>
              ...
              <div class="bg-white shadow p-2 flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href="<?= base_url('post/' . $row['post_slug']) ?>" class="mt-2 inline-flex items-center text-sm font-semibold text-blue-600">
                    selengkapnya
