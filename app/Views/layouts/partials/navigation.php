<header x-data="menuPublic()" x-init="fetchMenu()" id="navbar" class="backdrop-blur-xl transition-colors duration-500 bg-white/75 sticky top-0 z-40 font-semibold leading-6 shadow-md w-full">
                    ...
                    <div class="relative group" x-data="{ hover: false }">
                        ...
                        <template x-if="menu.children.length === 0">
                            <a
                                ...
                                class="inline-flex items-center px-4 py-2 text-sm font-medium  hover:text-blue-600"
                                :class="menu.active ? 'text-blue-600 font-semibold ' : 'text-gray-700'">
                        ...
                        <template x-if="menu.children.length > 0">
                            <div @mouseenter="hover = true" @mouseleave="hover = false">
                                <button
                                    ...
                                    class="inline-flex items-center px-4 py-2 text-sm font-medium  hover:text-blue-600 focus:outline-none"
                                    :class="menu.active ? 'text-blue-600 font-semibold' : 'text-gray-700'">
                                    <span x-text="menu.menu_title"></span>
                                ...
                                <div
                                    class="absolute left-0 mt-2 w-40 bg-white shadow-lg z-50 rounded-b-md"
                                    x-show="hover"
                                    x-transition>
                                    <template x-for="child in menu.children" :key="child.id">
                                        <a
                                            ...
                                            :class="child.active ? 'text-blue-600 font-semibold' : 'text-gray-700'"></a>
                                    </template>
                                </div>
                            </div>
                        </template>
                    ...
                    <div x-data="{ subOpen: false }">
                        ...
                        <template x-if="menu.children.length === 0">
                            <a
                                :href="menu.menu_type === 'link' ? menu.menu_url : base_url + menu.menu_url"
                                :target="menu.menu_target"
                                class="inline-flex text-sm font-medium text-gray-700 hover:text-blue-600"
                                :class="menu.active ? 'text-blue-600 font-semibold' : ''">
                                <span x-text="menu.menu_title"></span>
                            </a>
                        </template>
                                ...
                                <div x-show="subOpen" x-transition class="pl-4">
                                    <template x-for="child in menu.children" :key="child.id">
                                        <a
                                            ...
                                            :target="child.menu_target"
                                            x-text="child.menu_title"
                                            :class="child.active ? 'text-blue-600 font-semibold' : ''"></a>
                                    </template>
                                </div>
                            </div>
                        </template>
